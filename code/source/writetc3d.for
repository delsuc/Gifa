...
C********************************************************
       subroutine writetc3d(spec,size1,size2,size3,name)
c IN	: spec,size1,size2,size3
c INOUT	: spec,size1,size2,size3,name
c
...
20       format(' filename: ')
        ...
        enddo
        if (max0.eq.0.0) then
         call gifaout('*** the file is empty!!!')
         return
        endif
        const = 2.0d0**28
        fact= const/max0
...
#d       write(*,*) 'writing compacted file'
...
100    format(1x,'max0='/G15.8/1x,'normalisation='/G22.15)
       write(tline,200) max0
       call gifaout(tline)
       write(*,*)
200    format(' maximum of spectral data:max0=',G15.8)
...
C**********************************************************
       ...
       integer   i
       character ib
       character*1 char0,chara,charca
       data char0/'0'/,chara/'a'/,charca/'A'/
     
       if (i.le.9) then 
        ib=char(ichar(char0)+i)
       elseif (i.le.35) then
        ib=char(ichar(charca)+(i-10))
       elseif (i.le.61) then
        ib=char(ichar(chara)+(i-36))
       elseif (i.eq.62) then
        ib=','
       else
        ib='.'
