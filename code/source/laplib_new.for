...
C OUT : B(P)
C
C mad 1999 first version
C MAD + TG   added extension over limits      mar 2002
...
C K = (log(KK_2)-log(KK_1))/(n-1)
      ...
      do j = 2, n-1            ! loop over data values
         kcurr = -KK1*exp(K*(j-1))
c#d         write(*,*) j, A(j), kcurr
         ...
         enddo
      enddo
C separate code for border

C j = 1 corresponds to 0..K1 on dampings
C bi = A1/bi (1-exp(-bi K1))
C COMPUTED HERE WITH LINEAR METRIC, as exp metric is not analytical !

      do i = 1, p            ! loop over time points
	      if (samp(i).gt.epsilon) then
               b(i) = b(i) + A(1)*(1-exp( -KK1*samp(i))) / samp(i)
		else
		    b(i) = b(i) + A(1)*KK1
		endif
      enddo
C j = N corresponds to KN..Inf on dampings
C bi = A1/bi * exp(-bi Kn)
C COMPUTED HERE WITH LINEAR METRIC, as exp metric is not analytical !

      do i = 1, p            ! loop over time points
	      if (samp(i).gt.epsilon) then
               b(i) = b(i) + A(1)*exp( -KK2*samp(i)) / samp(i)
		endif
      enddo
...
#include "constant.inc"
      ...
      do j = 2, sizeimage1d-1            ! loop over output values
...
C COMPUTED HERE WITH LINEAR METRIC, as exp metric is not analytical !
      ...
      do i = 1, sizedata
	      if (sampling(i).gt.epsilon) then
		    timage(1) = timage(1) +
     *             tdata(i)*(1-exp(-sampling(i) * dmin)) / sampling(i)
		else
		    timage(1) = timage(1) + tdata(i)*dmin
		endif
      enddo
C j = N corresponds to KN..Inf on dampings
C bi = A1/bi * exp(-bi Kn)
C COMPUTED HERE WITH LINEAR METRIC, as exp metric is not analytical !
      ...
      do i = 1, sizedata
         if (sampling(i).gt.epsilon) then
            timage(sizeimage1d) = timage(sizeimage1d) +
     *        tdata(i) * exp( -dmax*sampling(i)) / sampling(i)
         endif
      enddo
