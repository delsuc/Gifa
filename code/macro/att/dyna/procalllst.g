...
;     disp : dispersion experiments : log(v(cpmg=0) / v(cpmg))
;     J 
...
if (!$arg) then
  ...
  formbox 'Processing tool for Dynamics Analysis' \
   ...
   'Type of data' enum 'R1,R2,NOE,T1,T2,J,disp' dt_tp data_typ \
...
; write header
  if (($data_typ s= "R1") | ($data_typ s= "T1") | ($data_typ s= "J")) then
      ...
      fprint $out "# RESNUM NAME VALUE VALUE_ERROR"
  elsif    ($data_typ s= 'disp') then
      set fractil = 'Non relevant'
      message "Enter CMPG constant time :"
      set Tconstant := $_
      if ($Tconstant <= 0) error "Should not be negative or null"
      fprint $out "# RESNUM { VALUE VALUE_ERROR }"
  else
 ...
 set header_disp = "not yet"
  while (!eof($inp))
    set uu = (%+1) inprogress $uu
    if (head($line) s= "Peak") then
      set dbno = (head(tail($line)))
      readcurve.g $nbexp $inp
      if ($data_typ s= "disp" & $header_disp s= "not yet") then     ; special header for dispersion exp
          set header_disp = "done"
          set list = "# List of nu value :"
          for i = 1 to $nbexp
             if ($paramx[$i] != 0)   set list = ($list; $paramx[$i])
          endfor
          fprint $out $list
      endif
      if ($returned == 1) then
